...
- The repo also contains a **Jekyll GitHub Pages microsite** in `docs/`; its README-derived data file is generated by `scripts/sync-pages-data.js`.
...
- `docs/` — GitHub Pages source, including `_config.yml`, `index.html`, and `_data/readme.json`.
...
The GitHub Pages microsite reads structured data from `docs/_data/readme.json`. Refresh that data from the repository `README.md` with:
```
npm run sync:pages
```
...
- After editing `README.md` content used by the GitHub Pages microsite, run `npm run sync:pages`.
...
- For microsite changes: edit files in `docs/`; if the content comes from `README.md`, run `npm run sync:pages` afterward.
...
- `scripts/sync-content-shared.js`, `scripts/sync-pages-data.js`, `scripts/clean-packages.js`
